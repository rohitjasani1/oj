...
function decideToShowFrontWidget() {
    ...
    $session = &nitroGetSession();
    if (empty($session['NitroRenderTime']) || empty($session['NitroNameOfCacheFile']) || empty($_GET['cachefile']) || $session['NitroNameOfCacheFile'] != $_GET['cachefile']) return false;
...
function serveCacheIfNecessary() {
	...
	if (file_exists($cachefile) && (time() - $nitrocache_time) < filemtime($cachefile)) {
		...
		$session['NitroNameOfCacheFile'] = base64_encode(generateNameOfCacheFile());
...
function minifyHtmlIfNecessary($html) {
	if (isNitroEnabled() && getNitroPersistence('Mini.Enabled') && getNitroPersistence('Mini.HTML')) {	
