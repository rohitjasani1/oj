...
// DEFINE ORDER NUMBER
$order_details_order_id = (int)$this->session->data['order_id'];

// GET SUB-TOTAL
$order_details_sub_total = $this->db->query("SELECT value FROM " . DB_PREFIX . "order_total where order_id = '$order_details_order_id' and code = 'sub_total'");
$order_details_sub_total = $order_details_sub_total->row['value'];

// GET COUPON CODE DETAILS
$idev_coupon_code = null;
$idev_coupon_used = null;
$idev_coupon_code_amount = 0;
$idev_coupon_code_details = $this->db->query("select coupon_id, amount from " . DB_PREFIX . "coupon_history where order_id = '$order_details_order_id'");

if ($idev_coupon_code_details->num_rows > 0) {
$idev_coupon_code = $idev_coupon_code_details->row['coupon_id'];
$idev_coupon_code_amount = $idev_coupon_code_details->row['amount'];
// -------------------------------------------------------------------
$idev_coupon_code_additional = $this->db->query("select code from " . DB_PREFIX . "coupon where coupon_id = '$idev_coupon_code'");
$idev_coupon_used = $idev_coupon_code_additional->row['code'];
}

// GET CUSTOMER IP
$order_details_ip_query = $this->db->query("select ip from " . DB_PREFIX . "order where order_id = '$order_details_order_id'");
$order_details_ip_address = $order_details_ip_query->row['ip'];

// CALCULATE SUB-TOTAL
$idev_sub_total = $order_details_sub_total + $idev_coupon_code_amount;

$url = "http://www.ornateaffiliate.com/sale.php?profile=64&idev_saleamt=$idev_sub_total&idev_ordernum=$order_details_order_id&coupon_code=$idev_coupon_used&ip_address=$order_details_ip_address";
$ch = curl_init();
...
// iDevAffiliate--End
		...
		if (isset($this->session->data['order_id'])) {
			...
			//$this->model_checkout_order->sendSmsEmailForOrder($this->session->data['order_id'], $this->config->get('cod_order_status_id'));
			
			$message_order = '{firstname} {lastname} {order_id}';
			
				$storename = 'ornate jewels';
				if($message_order != ""){
					$this->load->model('libraries/jossms');
					$gateway = 'mvaayoo';
					$parsing = array ( '{firstname}', '{lastname}', '{email}', '{telephone}','{order_id}' );
					$replace = array ( $this->db->escape($data['orderDetails']['firstname']), $this->db->escape($data['orderDetails']['lastname']), $this->db->escape($data['email']), $this->db->escape($data['orderDetails']['telephone']),$this->session->data['order_id'] );
					...
					$phone_number = $this->model_libraries_jossms->getConvertPhonePrefix($this->db->escape($data['orderDetails']['telephone']),$isoc);
