...
//print_r($sql);exit;
	...
	//public function getTOTALPRODUCTWITHfILTERdATA(){
	//	$sql="slect count (distinct p.product_id) AS TOTAL";
		
	//	$sql .= "Left JOin ".DB_PREFIX."product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " .DB_PREFIX. "product_to_store p2s ON (p.product_id= p2s.product_id) where p.product_id='".$this->id."' ";
...
//print_r(">".$query->row['total']);
	...
	public function getTotalProductSpecials() {
        ...
        $query = $this->db->query("	SELECT * FROM `oc_product_to_category` pc LEFT JOIN oc_category_description cd on cd.category_id=pc.category_id left join oc_category c on c.category_id=cd.category_id WHERE pc.product_id='".$product_id."' and c.parent_id=0");
        return $query->row;
        }
