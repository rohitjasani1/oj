...
//print_r($product_info);
		if ($product_info) {
			...
			if ($product_info['image']) {
				$data['thumb1'] = $this->model_tool_image->resize($product_info['image'], 90,90);
			} else {
				$data['thumb1'] = '';
			}
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'thumb' => $this->model_tool_image->resize($result['image'], 550,550),
					'thumb1' => $this->model_tool_image->resize($result['image'], 1000, 1000)

				);
			}
			//certificate.jpg
			$data['lastthumb'] =  $this->model_tool_image->resize('catalog/2.jpg', 90, 90);
			$data['lastimage'] =  $this->model_tool_image->resize('catalog/2.jpg', 400, 400);
			
			
			$data['lastthumb1'] =  $this->model_tool_image->resize('catalog/certificate.jpg', 90, 90);
		...
		public function pinCheck() {
        	...
        	if(isset($_POST['pincode'])){
        		...
        		if(isset($pin['id'])){
        			...
        			if($service == '1'){
        				echo "<div id='msg1'><label><img src='image/pincode/l.png' width='15' height='15' alt='pincode'>Delivery option for: ".$_SESSION['pincode']."</label>";
        				echo "<p>".$_SESSION['pin_check_result']."</p>
					 <div style='margin:8px 0px 18px 121px;'><input type = 'button'  onclick = 'showform1()' value='Change' class='pin-check' /></div></div>
        				...
        				echo "<div class='msg1'><img src='image/pincode/l.png' width='15' height='15'>Delivery option for: ".$_SESSION['pincode']."&nbsp;&nbsp;<form style='display: inline;'><input type = 'button'  onClick = 'showform1()' value='Change' class='pin-check' /></form><br />";
