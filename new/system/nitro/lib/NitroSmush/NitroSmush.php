    ...
    private $_config = array(
        ...
        '~\.jpe?g$~i' => array(
            'jpegoptim' => array(
                ...
                'extension' => 'jpg',
                'settings' => array(
                    '{EXE} --dest={DESTINATION_DIR} -o -q --strip-all --all-normal {SOURCE} 2> {STDERR}',
                    '{EXE} --dest={DESTINATION_DIR} -o -q --strip-all --all-progressive {SOURCE} 2> {STDERR}'
                )
            ),
            'jpegtran' => array(
                'linux32' => '',
        ...
        '~\.png$~i' => array(
            ...
            'pngcrush' => array(
                ...
                'extension' => 'png',
                'settings' => array(
                    '{EXE} -z {FOR0-3} -w {FOR32} -reduce -plte_len 1 -ow -s -d {DESTINATION_DIR} -force {SOURCE} 2> {STDERR}',
                    '{EXE} -reduce -brute -ow -s -d {DESTINATION_DIR} -force {SOURCE} 2> {STDERR}',
                )
            ),
            // 'pngout' => array(
            //     'linux32' => '',
