...
// FixFilter END
        ...
        public function getProductsLiveSearch($data) {
...
//print_r($sql);exit;
		...
		*/
		...
		$sql = "SELECT tag FROM tag where UCASE(tag) like '".strtoupper($data)."%'";
		//print_r($sql);
		//$sql .=	" like '".strtoupper($data['filter_filter'])."%'";
		...
		$areaString[]=$row['tag'];
		//	print_r($row['tag']);
	//	if (!in_array($areaString,$row['tag'])){
	//		$areaString.=$row['tag'];
	//	}
		}
		//exit;
		//$areaString = substr($areaString,0,strlen($areaString)-1);
		return $areaString;
	...
	public function getTotalProductSpecials() {
        ...
        public function havequestion($data){
           // echo 'sdf';
            ...
            $mobile=$data['mobile'];
            $detail=$data['detail'];
            $this->db->query("insert into " . DB_PREFIX . "havequestion set name='".$this->db->escape($name)."', email='".$this->db->escape($email)."',mobile='".$this->db->escape($mobile)."',detail='".$this->db->escape($detail)."',date_added=NOW()");
            return true;
        }
