...
class ModelSaleKdtrackerOrder extends Model {
    ...
    public function editTrackingNumber($order_id, $tracking_number) {
        ...
        if (count($query->rows) > 0) {
            $this->db->query("UPDATE " . DB_PREFIX . "kdtracker SET `tracking_number` = '" . $tracking_number . "',`tracking_status`='',`tracking_info`='',`ship_company`=''  WHERE `order_id` = '" . $order_id . "'");
        } else {
