...
function nitroCDNResolve($data, $real_url = '') {
    //if (!passesPageCacheValidation()) return $data; // Deprecated since NitroPack 2.3.9,and will be removed in future versions
    if (!empty($_GET["route"]) && strpos($_GET["route"], "feed/") === 0) return $real_url . $data;
...
function clearFTPPersistence() {
	...
	unset($session['nitro_ftp_persistence']);
}
